...
// PRIORITY 3: Process Screening Forms (Lowest Priority) - Only if not in physical exam or blood collection
foreach ($screening_forms as $screening_info) {
    ...
    $history_entry = [
        'no' => $counter,
        'donation_date' => $screening_info['created_at'] ?? $donor_info['submitted_at'] ?? date('Y-m-d H:i:s'),
        'surname' => $donor_info['surname'] ?? 'N/A',
        'first_name' => $donor_info['first_name'] ?? 'N/A',
        'gateway' => ($donor_info['registration_channel'] ?? '') === 'Mobile' ? 'Mobile' : 'PRC Portal',
...
$all_processed_donors = array_merge($donors_with_blood, $donors_with_physical);
foreach ($donor_forms as $donor_info) {
    ...
    $has_screening = false;
    foreach ($screening_forms as $screening) {
        if ($screening['donor_form_id'] == $donor_id) {
            $has_screening = true;
            break;
        }
    }
    ...
    $history_entry = [
        'no' => $counter,
        'donation_date' => $donor_info['submitted_at'] ?? date('Y-m-d H:i:s'),
        'surname' => $donor_info['surname'] ?? 'N/A',
        'first_name' => $donor_info['first_name'] ?? 'N/A',
        'gateway' => ($donor_info['registration_channel'] ?? '') === 'Mobile' ? 'Mobile' : 'PRC Portal',
...
if (empty($donor_history)) {
    ...
    foreach ($donors_data as $donor) {
        $history_entry = [
            ...
            'volume' => 'Pending', // No volume yet as no blood collection
            'collection_id' => null,
...
<head>
    ...
    <title>Red Cross Staff Dashboard - Medical Review History</title>
...
<body class="light-mode">
            ...
            <main class="col-md-9 col-lg-10 main-content">
                <div class="content-wrapper">
                    <div class="welcome-section">
                        <h2 class="welcome-title">Medical Review History</h2>
                    </div>
    ...
    <!-- Medical Review Modal (Fixed Size Design) -->
            ...
            <div class="modal-content" style="border: none; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); height: 65vh; display: flex; flex-direction: column;">
                <div class="modal-header border-0" style="background: linear-gradient(135deg, #b22222 0%, #8b0000 100%); border-radius: 12px 12px 0 0; padding: 1rem 1.5rem; flex-shrink: 0;">
                    <div class="d-flex align-items-center w-100">
                        ...
                        <div class="flex-grow-1">
                            <h5 class="modal-title text-white mb-1">Medical History Interview Complete</h5>
                            <p class="text-white-50 mb-0 small">Awaiting screening</p>
                        </div>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close" style="filter: brightness(0) invert(1);"></button>
                    </div>
                </div>
                <div class="modal-body flex-grow-1" style="padding: 1.5rem; overflow: hidden;">
                    ...
                    <div class="status-banner mb-4 p-4" style="background: linear-gradient(90deg, #f8e6e6 0%, #f0d0d0 100%); border-radius: 12px; border-left: 5px solid #b22222;">
                        <div class="row align-items-center">
                            <div class="col-md-8">
                                ...
                                <div class="next-step">
                                    <strong class="text-dark">Next Step:</strong> 
                                    <span class="text-muted">Staff to fill up screening and hospital history form</span>
                                </div>
                            </div>
                            <div class="col-md-4 text-center">
                                <div class="next-icon" style="width: 80px; height: 80px; background: rgba(178, 34, 34, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto;">
                    ...
                    <div class="info-section">
                        <h6 class="section-title mb-3" style="color: #495057; font-weight: 600; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 0.5px;">
                            <i class="fas fa-user me-2" style="color: #6c757d;"></i>Registration Information
                        </h6>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="info-item mb-3" style="background: #f8f9fa; padding: 1rem; border-radius: 8px; border: 1px solid #e9ecef;">
                                    <label class="detail-label">Registration Date</label>
                                    <div class="detail-value" id="medicalReviewModalDate">August 12, 2025</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-item mb-3" style="background: #f8f9fa; padding: 1rem; border-radius: 8px; border: 1px solid #e9ecef;">
                                    <label class="detail-label">Donor Name</label>
                                    <div class="detail-value" id="medicalReviewModalName">Jam Rat</div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.querySelectorAll('.view-history-btn').forEach(button => {
                button.addEventListener('click', function() {
                    try {
                        ...
                        // Show different modals based on stage (includes medical review)
                        if (stage === 'blood_collection') {
                            ...
                            modal.show();
                        } else if (stage === 'medical_review') {
                            populateMedicalReviewModal(historyData);
                            const modal = new bootstrap.Modal(document.getElementById('medicalReviewModal'));
                            modal.show();
                        }
            ...
            function populateScreeningOnlyModal(data) {
                if (data.donation_date) {
                    const date = new Date(data.donation_date);
                    document.getElementById('screeningModalDate').textContent = date.toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long', 
                        day: 'numeric'
                    });
                ...
                const fullName = (data.first_name || '') + ' ' + (data.surname || '');
                document.getElementById('screeningModalName').textContent = fullName.trim() || 'N/A';
            }
            
            // Function to populate medical review modal
            function populateMedicalReviewModal(data) {
                if (data.donation_date) {
                    const date = new Date(data.donation_date);
                    document.getElementById('medicalReviewModalDate').textContent = date.toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long', 
                        day: 'numeric'
                    });
                ...
                const fullName = (data.first_name || '') + ' ' + (data.surname || '');
                document.getElementById('medicalReviewModalName').textContent = fullName.trim() || 'N/A';
            }
