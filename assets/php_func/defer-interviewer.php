...
function logError($message) {
    error_log("Defer Interviewer Error: " . $message);
...
function calculateTemporaryDeferredText($deferral_type, $duration) {
    // For temporary deferral, set to today's date (no days calculation)
    if ($deferral_type === 'Temporary Deferral') {
        return 'Temporarily Deferred';
    } else if ($deferral_type === 'Permanent Deferral' && $duration) {
        // For permanent deferral, use duration if provided
...
/**
 * Create eligibility record for defer action (Interviewer version)
 * Used when deferring a donor from the interviewer dashboard
 * Modified logic:
 * - Temporary deferred: no days would be used (duration = null, no end_date calculation)
...
function createDeferEligibilityRecord($data) {
    try {
        ...
        // Calculate end date based on deferral type
        $end_date = null;
        
        // MODIFIED LOGIC: Temporary deferred - no days selector, but set to today's date by default
        if ($deferral_type === 'Temporary Deferral') {
            // Set end_date to today's date (no days calculation, just today)
            $end_date = date('Y-m-d H:i:s');
            $duration = null; // Force duration to null for temporary deferrals
        } 
        // Permanent deferred - should pass normally (with days if provided)
        else if ($deferral_type === 'Permanent Deferral' && $duration) {
        ...
        error_log("Defer interviewer eligibility record created successfully for donor_id: $donor_id, type: $deferral_type, status: $status, duration: " . ($duration ?: 'N/A') . ", medical_history_id: " . ($medical_history_id ?: 'N/A') . ", physical_exam_id: " . ($physical_exam_id ?: 'N/A'));
...
try {
    ...
    if ($method === 'POST') {
        ...
        error_log("Defer Interviewer - Received data: " . json_encode($data));
        error_log("Defer Interviewer - Screening form data received: " . json_encode($data['screening_form_data'] ?? 'No screening form data'));
    ...
    error_log("Defer Interviewer error: " . $e->getMessage());
    echo json_encode(["success" => false, "error" => "Internal server error: " . $e->getMessage()]);
} catch (Error $e) {
    error_log("Defer Interviewer fatal error: " . $e->getMessage());
