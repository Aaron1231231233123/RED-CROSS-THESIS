...
$primaryKeyMap = [
    ...
    'physical_examination' => 'physical_exam_id',
    'blood_collection' => 'blood_collection_id',
...
/**
 * Update access value for a given table.
 */
...
function updateAccessValue(string $table, int $value, array $filters = [])
{
    ...
    if (empty($filter)) {
        // Fall back to safe filter to avoid full table updates
        $filter = '?access=gte.0';
    }

    $url = SUPABASE_URL . "/rest/v1/{$table}{$filter}";
    // Updating without filters applies to all rows - intentional for global locking.
...
/**
 * Fetch current access value for a given table.
 */
