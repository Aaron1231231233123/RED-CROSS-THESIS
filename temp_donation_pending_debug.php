...
try {
    ...
    // DEBUG: Special debugging for donor 185 - eligibility check
    if ($donorId == 185) {
        error_log('DEBUG DONOR 185: Checking eligibility status');
        if (isset($eligibilityData) && !empty($eligibilityData)) {
            error_log('DEBUG DONOR 185: Eligibility data found: ' . json_encode($eligibilityData[0]));
        } else {
            error_log('DEBUG DONOR 185: No eligibility data found');
        }
    }
    ...
    // DEBUG: Special debugging for donor 185 - about to continue
    if ($donorId == 185) {
        error_log('DEBUG DONOR 185: About to continue due to eligibility status: ' . $eligibilityStatus);
    }
    continue;
                        }
                    }
    ...
    // DEBUG: Special debugging for donor 185 - decline/deferral check
    if ($donorId == 185) {
        error_log('DEBUG DONOR 185: Checking for decline/deferral status');
        error_log('DEBUG DONOR 185: hasDeclineDeferStatus = ' . ($hasDeclineDeferStatus ? 'true' : 'false'));
        if ($hasDeclineDeferStatus) {
            error_log('DEBUG DONOR 185: Decline/deferral type: ' . $declineDeferType);
        }
    }
    if ($hasDeclineDeferStatus) {
                    if (in_array($donorId, [176, 169, 170, 144, 140, 142, 135, 120])) {
    ...
    // DEBUG: Special debugging for donor 185 - about to continue due to decline/deferral
    if ($donorId == 185) {
        error_log('DEBUG DONOR 185: About to continue due to decline/deferral status: ' . $declineDeferType);
    }
    continue; // Skip this donor - they have a final decline/deferral status
                }
                
                // PENDING STATUS DETERMINATION LOGIC
    // DEBUG: Special debugging for donor 185 - status determination
    if ($donorId == 185) {
        error_log('DEBUG DONOR 185: Determining final status');
        error_log('DEBUG DONOR 185: medicalHistoryCompleted = ' . ($medicalHistoryCompleted ? 'true' : 'false'));
        error_log('DEBUG DONOR 185: screeningCompleted = ' . ($screeningCompleted ? 'true' : 'false'));
        error_log('DEBUG DONOR 185: physicalExamCompleted = ' . ($physicalExamCompleted ? 'true' : 'false'));
    }
